    ...
    <header>
      ...
      <section>
        ...
        <form action="" method="get"> 
          <input type="text" placeholder="Izenburua, idazlea..." name="busqueda" id="busqueda">
          <button type="submit" name="enviar"><i class="fa-solid fa-magnifying-glass"></i></button>
        </form>
        </div>
    ...
    <main>
      ...
      <?php
        if(isset($_GET['enviar'])){
                ...
                $consulta= $miPDO->prepare('SELECT * FROM libro WHERE titulo LIKE "%' . $busqueda . '%" OR escritor LIKE "%' .$busqueda. '%" OR etiqueta LIKE "%' .$busqueda. '%";');
                $consulta->execute();
                $OK= $consulta->fetchAll();

                if ($OK) {
                  echo '<section>';
                  foreach ($OK as $position => $book) {
                    echo '<div class="book-container">';
                    if ($book['portada'] === '') {
                    echo '<img src="../src/img/books/default.jpg">';
                    } else {
